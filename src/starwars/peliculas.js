...
// for (let i = 0; i < 5; i++) {
    ...
    fetchData(API, function(error1, data1){
        ...
        fetchData(data1.films, function(error2, data2){
            if(error2) return console.log(error2);
            fetchData(data2.results[1].url, function(error3, data3){
                const { title } = data3
                const { episode_id } = data3
                const { director } = data3
                const { producer } = data3
                const { release_date } = data3
